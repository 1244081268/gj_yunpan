...
// 					currentPath = path;
	...
	function joinUrl(url){
		...
		return host + "share?shareUrl=" + url;
	}
	/**打开我的分享*/
			...
			if (data.success) {
				...
				if(type == "image"){
					...
					$.each(data.data, function() {
						...
						$("#"+ typeName).append('<a href="file/openFile?'+url+'" data-lightbox="roadtrip" title="'+this.fileName+'"><img alt="'+this.fileName+'" style="margin: 10px" src="file/openFile?'+url+'" width="150" height="150"></a>')
					});
				}else{
	...
	function openFile(obj) {
		...
		if (fileType == "folder-open") {
			...
			$(obj).attr({"href":"file/openFile?"+url,"data-lightbox":"test","data-title":fileName});
			return true;
		} else if(fileType.indexOf("office") >= 0){
			$.post("file/openOffice", {
			...
			layer.open({
				type:2,
				title:'播放',
				content:'file/openAudioPage?' + url,
				shade: [0],
				area: ['440px', '120px'],
				offset: 'rb', //右下角弹出
			});
