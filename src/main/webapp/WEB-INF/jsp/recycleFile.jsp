...
<head>
	...
	function revertFile(obj){
		...
		$.post("${pageContext.request.contextPath }/file/revertDirectory", {"fileId":id}, function(data){
			layer.msg(data.msg);
	...
	function revertAllFiles(){
		...
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath }/file/revertDirectory",
			data:{
				"fileId":fileId,
			},
	...
	function delAllFiles(){
		layer.confirm('确认清空回收站？', {
			  btn: ['确认','返回'] //按钮
				...
				$.ajax({
					type:"POST",
					url:"${pageContext.request.contextPath }/file/delAllRecycle",
					data:{
					},
					success:function(data){
						layer.msg(data.msg);
	...
	function delFile(obj){
		layer.confirm('确认删除？', {
			  btn: ['确认','返回'] //按钮
				...
				var id = $(obj).parent().parent().children(".oneCheck").attr("id");
				$.post("${pageContext.request.contextPath }/file/delRecycle", {"fileId":id}, function(data){
					layer.msg(data.msg);
					window.location.reload();
				});
			}, function(){
